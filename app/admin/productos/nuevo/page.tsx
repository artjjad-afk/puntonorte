...
export default function NuevoProducto() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name || !form.price || !form.description || form.images.length === 0) {
      setError('Completa todos los campos requeridos y agrega al menos una imagen.')
      return
    }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f4f2f0', fontFamily: 'Arial, sans-serif' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: '26px', fontWeight: '800', color: '#211f1e', margin: '0 0 28px', letterSpacing: '-0.5px' }}>Nuevo Producto</h1>
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              ...
              <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', border: '1px solid #e8e5e2' }}>
                ...
                <div style={{ display: 'grid', gap: '16px' }}>
                  <div>
                    ...
                    <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="Ej: Camisa Polo Negra" style={inputStyle}
                      onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                  </div>
                  <div>
                    <label style={labelStyle}>Descripción *</label>
                    <textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Describe el producto..." rows={4}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div>
                      ...
                      <input type="number" step="0.01" min="0" value={form.price} onChange={e => set('price', e.target.value)} placeholder="0.00" style={inputStyle}
                        onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                    </div>
                    <div>
                      <label style={labelStyle}>Precio original (opcional)</label>
                      <input type="number" step="0.01" min="0" value={form.originalPrice} onChange={e => set('originalPrice', e.target.value)} placeholder="0.00" style={inputStyle}
                        onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', border: '1px solid #e8e5e2' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
                  <input value={imageInput} onChange={e => setImageInput(e.target.value)} placeholder="https://... URL de la imagen"
                    style={{ ...inputStyle, flex: 1 }} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addImage())}
                    onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                  <button type="button" onClick={addImage} style={{ padding: '11px 18px', background: '#c1692b', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {form.images.map((img, i) => (
                    <div key={i} style={{ position: 'relative', width: '80px', height: '80px', borderRadius: '10px', overflow: 'hidden', border: '2px solid #e8e5e2' }}>
                      ...
                      <button type="button" onClick={() => removeImage(i)} style={{ position: 'absolute', top: '2px', right: '2px', width: '20px', height: '20px', borderRadius: '50%', background: 'rgba(220,38,38,0.9)', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <X size={12} />
                      </button>
                    </div>
                  ))}
                </div>
                <p style={{ fontSize: '12px', color: '#7a7675', marginTop: '10px' }}>Agrega URLs de imágenes. La primera será la imagen principal.</p>
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                      <input value={sizeInput} onChange={e => setSizeInput(e.target.value)} placeholder="Ej: S, M, L" style={{ ...inputStyle, flex: 1 }}
                        onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addSize())}
                        onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                      <button type="button" onClick={addSize} style={{ padding: '11px 14px', background: '#211f1e', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '700' }}><Plus size={14} /></button>
                    </div>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      {form.sizes.map(s => <span key={s} onClick={() => set('sizes', form.sizes.filter(x => x !== s))} style={{ padding: '4px 12px', background: '#211f1e', color: '#fff', borderRadius: '100px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>{s} ×</span>)}
                    </div>
                  </div>
                  <div>
                    <label style={labelStyle}>Colores</label>
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                      <input value={colorInput} onChange={e => setColorInput(e.target.value)} placeholder="Ej: Negro, Rojo" style={{ ...inputStyle, flex: 1 }}
                        onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addColor())}
                        onFocus={e => (e.target.style.borderColor = '#c1692b')} onBlur={e => (e.target.style.borderColor = '#e8e5e2')} />
                      <button type="button" onClick={addColor} style={{ padding: '11px 14px', background: '#211f1e', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '700' }}><Plus size={14} /></button>
                    </div>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      {form.colors.map(c => <span key={c} onClick={() => set('colors', form.colors.filter(x => x !== c))} style={{ padding: '4px 12px', background: '#c1692b', color: '#fff', borderRadius: '100px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>{c} ×</span>)}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                  {[
                    ...
                    <label key={key} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', userSelect: 'none' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#393738', fontWeight: '500' }}>{label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                <Link href="/admin/productos" style={{ padding: '13px 24px', border: '1.5px solid #e8e5e2', borderRadius: '10px', color: '#393738', textDecoration: 'none', fontWeight: '700', fontSize: '14px' }}>
                  Cancelar
                </Link>
                <button type="submit" disabled={loading} style={{ padding: '13px 32px', background: loading ? '#7a7675' : '#c1692b', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: '800', fontSize: '14px', cursor: loading ? 'not-allowed' : 'pointer', transition: 'background .2s', fontFamily: 'Arial, sans-serif' }}>
