...
function StockBadge({ stock, reponiendo, onReponer }: {
  stock: number; reponiendo: boolean; onReponer: (qty: number) => void
}) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <button onClick={() => setOpen(o => !o)} title="Reponer stock"
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -6, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.95 }} transition={{ duration: 0.15 }}
            ...
            <p style={{ margin: '0 0 6px', fontSize: 11, color: 'rgba(148,163,184,0.6)', fontFamily: 'var(--font-mono)' }}>REPONER STOCK</p>
            <p style={{ margin: '0 0 8px', fontSize: 12, color: 'rgba(148,163,184,0.5)' }}>Actual: <span style={{ color, fontWeight: 700 }}>{stock}</span></p>
            <div style={{ display: 'flex', gap: 6 }}>
              <input type="number" min="1" value={qty} onChange={e => setQty(e.target.value)} onClick={e => e.stopPropagation()}
              ...
              <button disabled={reponiendo} onClick={e => { e.stopPropagation(); onReponer(parseInt(qty) || 0); setOpen(false) }}
...
export default function AdminProductos() {
  ...
  const handleReponer = async (id: number, currentStock: number, qty: number) => {
    ...
    setReponiendo(id)
    try {
      const res = await fetch(`/api/products/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ stock: currentStock + qty }) })
      if (!res.ok) throw new Error()
      fetchProducts()
  ...
  const filtered = products
    .filter(p => filter === 'all' ? true : filter === 'active' ? p.active : filter === 'inactive' ? !p.active : p.stock <= 3)
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        {loading ? (
          ...
          <div style={{ padding: 64, textAlign: 'center', color: 'rgba(148,163,184,0.3)' }}>
            ...
            <p style={{ margin: '0 0 4px', fontSize: 15, fontWeight: 600, color: 'rgba(148,163,184,0.4)' }}>{search ? `Sin resultados para "${search}"` : 'Sin productos'}</p>
            {!search && (
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 80px 140px 80px 100px', gap: 12, padding: '11px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <AnimatePresence>
              {filtered.map((p, i) => (
                <motion.div key={p.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                  className="prod-row" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 80px 140px 80px 100px', gap: 12, padding: '13px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                  {/* Producto */}
                  ...
                  {/* Precio */}
                  <span style={{ fontSize: 14, fontWeight: 800, color: '#f97316', fontFamily: 'var(--font-mono)' }}>${p.price.toFixed(2)}</span>
                  {/* Stock */}
                  <StockBadge stock={p.stock} reponiendo={reponiendo === p.id} onReponer={(qty) => handleReponer(p.id, p.stock, qty)} />
                  {/* Estado */}
                  ...
                  <div style={{ display: 'flex', gap: 5 }}>
                    <Link href={`/admin/productos/${p.id}/editar`} title="Editar" style={{ width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.2)', color: '#60a5fa', textDecoration: 'none' }}>
                      <Pencil size={13} />
                    </Link>
                    <button onClick={() => toggleActive(p.id, p.active)} title={p.active ? 'Ocultar' : 'Mostrar'} style={{ width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.15)', color: 'rgba(148,163,184,0.6)', cursor: 'pointer' }}>
                      {p.active ? <EyeOff size={13} /> : <Eye size={13} />}
                    </button>
                    <button onClick={() => handleDelete(p.id)} disabled={deleting === p.id} title="Eliminar" style={{ width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', color: '#f87171', cursor: deleting === p.id ? 'not-allowed' : 'pointer', opacity: deleting === p.id ? 0.5 : 1 }}>
                      <Trash2 size={13} />
                    </button>
                  </div>
                </motion.div>
